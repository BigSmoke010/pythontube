...
vdio_format = Radiobutton(root, variable=option, value=0, text='mp4', command=changevarz)
vdio_format.grid(row=0, column=0)


plylist_format = Radiobutton(root, variable=option, value=1, text='mp3', command=changevary)
...
def submitplaylist():
    ...
    for i in vd.videos:
        if vidtru == 0:
            ...
            i.streams.get_highest_resolution().first().download(output_path='downloads/')
            print('succesfully downloaded')
        else:
            print('Downloading : ' + i.title)
